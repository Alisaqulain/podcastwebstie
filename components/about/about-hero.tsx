...
export function AboutHero({ clips }: { clips: AmbientClip[] }) {
  ...
  return (
    <section className="relative isolate min-h-[min(100svh,42rem)] overflow-hidden border-b border-luxury-border sm:min-h-[min(92svh,44rem)]">
      ...
      <motion.div
        className="relative z-10 mx-auto flex min-h-[min(100svh,42rem)] w-full max-w-6xl items-center px-3 pb-14 pt-[max(5.5rem,env(safe-area-inset-top)+4.5rem)] sm:min-h-[min(92svh,44rem)] sm:px-6 sm:pb-20 sm:pt-24 md:pb-24 md:pt-28"
        initial={reduceMotion ? false : { opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
      >
        <div className="readable-content-panel w-full rounded-[1.35rem] px-4 py-6 sm:rounded-[1.75rem] sm:px-7 sm:py-9 md:max-w-3xl">
          ...
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-luxury-body sm:mt-6 sm:text-lg">
            {SITE.description}
          </p>
        </div>
      </motion.div>
